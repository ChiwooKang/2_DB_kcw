 ...
 * 
 * - 데이터를 조회(SLECT)하면 조건에 맞는 행들이 조회가 됨.
 * 이때, 조회된 행들의 집합을 "REULST SET" (조회 결과의 집합) 이라고 한다.
 * 
 * 
 * - RESULT SET은 0개 이상의 행을 포함할 수 있다.
 *  왜 0개? 조겐에 맞는 행이 없을 수도 있어서...
 * 
 * */
...
--> " " 쌍 따옴표는 특수문자, 대소문자, 기호 등을 구분하여 나타낼 때 사용하는 표기법
-- 		쌍따옴표 안에 작성되는 것을디 하나임을 의미한다
...
-- LIKE ESCAPE OPTION : 일반 문자로 처리할 '_ ' / '%'  앞에
