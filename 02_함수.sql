...
-- EMPLYOEE 테이블에서 사원명, 이메일, 이메일 중 '@(at)' 위치 조회
...
SELECT '절대값 같음'
FROM DUAL 
WHERE ABS(1O) = ABS(-10) ; -- WHERE절 함수 작성 가능
...
SELECT TRUNC(123.456) FROM DUAL; -- 소수점 아래 버림 
...
SELECT TO_CHAR(1234) FROM DUAL; -- 1,2,3,4 -> '1234'
...
SELECT SYSDATE FROM DUAL; -- 2022-09-01 11:21:21.000
...
-- 날짜 형식이 부적합합니다.
-- 년,월,일이 잘짜를 나타내는 패턴으로 인식이 안되서 오류 발생
...
--EMEPLOYEE 테이블에서 각 직원이 태어난 생년월일(1990년 05월 13일) 조회
SELECT EMP_NAME,
	TO_CHAR (TO_DATE( SUBSTR (EMP_NO , 1, INSTR(EMP_NO,'-') -1), 'RRMMDD') ,
		'YYYY"년"MM"월" DD"일"') AS 생년월일
FROM EMPLOYEE;

-- Y패턴 : 현재 세기(21세기 == 20XX년 == 20000년대)
-- R패턴 :  1세기를 기준으로 절반(50년) 이상인 경우 이전 세기(1900 년대)
--							 절반(50년) 미만인 경우 이전 세기(1900 년대)
