...
-- EMPLPOYEE 테이블에서 부서코드, 부서별 급여 합 조회
...
FROM EMPLOYEE
GROUP BY DEPT_CODE; --> DEPT_CODE가 같은 행 끼리 하나의 그릅이 됨
--  ORA-00937: not a single-group group function // 단일 그룹의 함수가 아니다
...
SELECT DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') 성별, 
	   ...
	   SUM(SALARY) "급여 합" 
FROM EMPLOYEE
GROUP BY  DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') -- 별칭 사용 x(별칭 해석이 아직 x)(SELECT 해석 X)
ORDER BY "인원 수"; -- 별칭 사용 가능(SELECT절 해석 완료 )
...
FROM EMPLOYEE
WHERE  EXTRACT(YEAR FROM HIRE_DATE) >= 20000
...
--EMPLOYEE 테이블에서 부서별로 같은 직급의 사원의 수를 조회
--(부서코드 오름차순, 직급코드 내림차순)
...
-- 부서별 평균 급여가 3백만원 이상인 부서를 조회(부서코드 오름 차순)
SELECT DEPT_CODE, AVG(SALARY)
FROM EMPLOYEE
-- WHERE SALARY >= 3000000 -->한 사람의 급여가 3백만 이상이라는 조건(요구사항 충족하지 않음)
GROUP BY DEPT_CODE
...
-- 서로 다른 테이블이지만 컬럽의 타입, 개수만 일치하면 집합 연산자 사용 가능!
